...
class pop3
{
    ...
    // 这个方法取得服务器端的返回信息并进行简单的处理：去掉最后的回车换行符，将返回信息保存在resp这个内部变量中。这个方法在后面的多个操作中都将用到。
    function outdebug($message)
    {
        echo htmlspecialchars($message) . "<br>\n";
        }
    
    //建立起与服务器的sock连接之后，然后服务器给予一定的回应, 我们可以做一个通过发送命令的方法:
    ...
    // 指定从服务器的返回中取多长的值做为命令返回的标识以及这个标识的正确值是什么。
    // 对于一般的pop操作来说，如果服务器的返回第一个字符为"+"，则可以认为命令是正确执行了。
    // 也可以用前面提到过的三个字符"+OK"做为判断的标识。
    // 下面介绍的几个方法则可以按照前述收取信件的对话去理解，因为有关的内容已经在前面做了说明，
    // 因此下面的方法不做详细的说明，请参考其中的注释：
 ...
 // 发送用户名及密码，登录到服务器
    ...
    function login($user, $password)
    {
        ...
        // 已经可以实现邮件的查看、收取、删除的操作，不过别忘了最后要退出，并关闭与服务器的连接，调用下面的这个方法：
        function Close()
        {
            if($this -> connection != 0)
            {
