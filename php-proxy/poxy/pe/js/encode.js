...
function encrypt(str,key){
    ...
    var n = str.length;
    for (var i=0; i<n; i++){
        e=str.charCodeAt(i)+key;
        estr+=String.fromCharCode(e);
    }
    return estr;
}

function decrypt(estr,key){
    var key;
    var str='';
    var n = estr.length;
    for (var i=0; i<n; ++i){
        s=estr.charCodeAt(i)-key;
...
function str2hex(str){    
	...
	var le = str.length;
    for (i=0; i<le; i++){
		r += (hexes[(str.substring(i,le).charCodeAt() >> 4)] + hexes[(str.substring(i,le).charCodeAt()&0xf)]);
	}
    return r;
}
