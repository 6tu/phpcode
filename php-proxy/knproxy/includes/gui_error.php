...
<p>The server met an error or was interrupted while trying to fulfill your request for <span style="font-family:monospace;"><?php echo $url;?></span>
...
<?php } else {
	...
	echo '<em class="a">knHTTP request:</em> ' . $knHTTP->url . '<em>(' . strlen($knHTTP->url) . ' bytes)</em><br>';
	...
	echo '<em class="a">knHTTP headers:</em> ' . $knHTTP->headers . '<em>(' . strlen($knHTTP->headers) . ' bytes)</em><br>';
	echo '<em class="a">knHTTP return body length:</em> <em>' . strlen($knHTTP->content) . ' bytes</em><br>';
	echo '<em class="a">knHTTP return body(html encoded):</em> ' . preg_replace('~<~','&lt;',$knHTTP->content) . '<br>';
};?>
