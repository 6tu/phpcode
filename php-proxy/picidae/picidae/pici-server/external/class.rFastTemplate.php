...
// PHP5 Port: Copyright  2008 Marguerie Jérémie <jiem78@homail.com>, All Rights Reserved.
// Perl Version: Copyright  1998 Jason Moore <jmoore@sober.com>, All Rights Reserved.
//
// RCS Revision
//   @(#) $Id: class.rFastTemplate.php,v 1.1.1.1 2001/09/21 14:34:18 flo-1 Exp $
//   $Source: /cvsroot/sfidirector/gui/public_html/class.rFastTemplate.php,v $
//
// Copyright Notice
//
...
class rFastTemplate {
   ...
   var $DEBUGFILE = '/opt/lampp/htdocs/pici-server/tmp/class.rFastTemplate.php.dbg';
   ...
   function &load ($file) {
      ...
      if(isset($this->DEBUGALL) AND $this->DEBUGALL)
      	
      	$debug = $this->DEBUGALL;
      
      elseif(isset($this->DEBUG['load']) AND $this->DEBUG['load'])
      	
      	$debug = $this->DEBUG['load'];
   ...
   function &parse_internal_1 ($tag, $rest = '') {
      ...
      elseif(isset($this->DEBUG['parse_internal_1']) AND $this->DEBUG['parse_internal_1'])
      	
      	$debug = $this->DEBUG['parse_internal_1'];
            ...
            while (preg_match ($this->REGEX_DYNEND, $rest, $dynend)) {
               ...
               if ($dynbeg[2] == $dynend[2]) {
                  ...
                  if ($this->TEMPLATE[$dynend[2]][clear]) {
                     ...
                     $this->TEMPLATE[$dynend[2]][results] = '';
   ...
   function parse_internal ($tag) {
      ...
      elseif(isset($this->DEBUG['parse_internal']) AND $this->DEBUG['parse_internal'])
      	
      	$debug = $this->DEBUG['parse_internal'];
      ...
      if (gettype($tag) == 'array') {
	    ...
	    $this->TEMPLATE[$tag]['loaded'] = 1;
         ...
         if ($this->DYNAMIC) {
            ...
            if (!isset($this->TEMPLATE[$tag]['parsed']) OR !$this->TEMPLATE[$tag]['parsed']) {
   ...
   function subst ($handle, $tag, $autoload = true) {
      ...
      elseif(isset($this->DEBUG['subst']) AND @$this->DEBUG['subst'])
      	
      	$debug = $this->DEBUG['subst'];
