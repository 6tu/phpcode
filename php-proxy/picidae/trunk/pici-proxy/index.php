...
pici-proxy is part of the picidae-project http://www.picidae.net
it shall serve as a proxy to a pici-server
Copyright (C) 2007  picidae.net by mathias jud and christoph wachter

this script is based on PHProxy of Abdullah Arif
...
/* ----------------------------------------------------------------------
    initial section (nothing to configure anymore... you'r done :-) )
   ---------------------------------------------------------------------- */

error_reporting(E_ALL);
$use_htaccess_rewrite = false;
...
/* ----------------------------------------------------
   ...
   make immediately a connection to the pici-server
   if the piciserver fails, show an error-picture
*/
...
function complete_url($url, $proxify = true)
{
    ...
    global $use_htaccess_rewrite;
    if ( $use_htaccess_rewrite ) $proxify = false ;
...
/*
   decode URL
   Not neede anymore
   URL shall look like this:
   {proxy_path}/path/to/resource.jpg               //with .htaccess
   {proxy_path}/index.php?q=/path/to/resource.jpg  //without .htaccess
*/
function encode_url($url)
{
    //@@@ better return just the relative url (the whole one is not needed)
...
{
    global $pici_server;
    return "http://" . $pici_server ."/" . str_replace(array('&amp;', '&#38;'), '&', rawurldecode($url));
}
...
{
    //echo ($_GET[$_config['get_form_name']] ."<br/>");
                ...
                case 'form':
                    if (isset($attrs['action']))
                    {
                        ...
                        if (!isset($attrs['method']) || strtolower(trim($attrs['method'])) === 'get')
                        {
                            ...
                            if ($use_htaccess_rewrite)
                            {
                            	$attrs['action'] = $attrs['action'];
                            }
                            else
                            {
                            	$extra_html = '<input type="hidden" name="' . $_config['get_form_name'] . '" value="' . encode_url($attrs['action']) . '" class="n" />';
                            	$attrs['action'] = '';
                            }
                            break;
                        }
