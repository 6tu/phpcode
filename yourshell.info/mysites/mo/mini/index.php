...
while (($file = readdir($dh)) !== false) {
    if(filetype($dir . $file) !== "dir"){
        ...
        if((time() - $ftime) > 1200 && strstr($file,'index.php') == false)  unlink($dir.$file);
        //echo "$file \n";
    }
}
closedir($dh);
header('Content-Type: text/html; charset=gb2312');
$form = '<html><head><title>在线修改S60V2 MINI6服务器</title></head><body><br /><br /><center><h3><a href="./operamini.zip" title="http://m.opera.com/OperaMini.sis?act=dl&tag=mini5s60_sdk2"/>S60V2 Opera mini6</a> 在线改服</h3><br /><small> 不 要 带 http://，小 于 30 个 字 符</small><form method="get" action="' . $_SERVER['PHP_SELF'] . '" >' .
        '代理地址: <input type="text" maxLength="30" size="40" name="url" value="" /> <input type="checkbox" name="socket" checked="checked" /> Socket<b>不使用国服</b>  <input type="submit" value="提交" /></form></body></html>';


if(!isset($_GET['url']) or empty($_GET['url'])) die($form);
if(isset($_GET['url']) && !empty($_GET['url'])){
$url = trim(ltrim($_GET['url']));
$url = str_replace(' ','%20',$url);
$url = str_replace('http://','',$url);
if(strlen($url) >= 30) die($form . "<b>网址太长，无效</b>\r\n<br />");
$cnser = bin2hex('mini5cn.opera-mini.net:80').'00000000';
$myser = bin2hex($url);
$n = strlen($cnser) - strlen($myser);
$zero = '';
for($i=0; $i< $n; $i++){
$zero .= 0;
}
$myser .= $zero ;
$hex = bin2hex (file_get_contents('OperaMini.app.bak'));
$hex = str_replace($cnser,$myser,$hex);
if(isset($_GET['socket']) && $_GET['socket'] == "on"){
$hex = str_replace(bin2hex('mini5cn.opera-mini.net:1080'),bin2hex('mini5.opera-mini.net:1080').'0000',$hex);
}
...
$fn = substr(md5($url),0,10).'_S60V2_OperaMini';
file_put_contents($fn.'.app',hex2bin($hex));
$test = new zip_file("$dir$fn.zip");
$test->add_files("$fn.app");
$test->add_files("readme.txt");
$test->create_archive();
echo $form .'<b>请尽快下载您的文件，文件可能在20分钟后被服务器删除</b><br /><br /><a href="' .$dir.$fn. '.zip">'.$fn.'</a>';
unlink("$fn.app");
if(strstr(file_get_contents('url.log'),$url) !== false) echo exit(0);

$log = $_SERVER["REMOTE_ADDR"] .'#'. date('Ymd His') .'#'. $url ."#" .$dir.$fn. '.zip'."\r\n==============\r\n\n";
$oldlog = file_get_contents('url.log');
file_put_contents('url.log',$log.$oldlog);
echo exit(0);
}
